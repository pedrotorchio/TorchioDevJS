...
function data2model(data) {
    let model = new src_1.Model(data.id);
    ...
    for (const tagName of ["tags", "synonyms"]) {
        if (data[tagName])
            model.meta[tagName] = new src_1.Tags(data[tagName]);
    }
    return model;
}
exports.data2model = data2model;
function data2image(data, baseUrl = "") {
    let main_image = data2model(data).copyInto(src_1.Image);
    ...
    main_image.meta.tags = new src_1.Tags(data.tags);
    main_image.meta.tags = new src_1.Tags(data.tags);
